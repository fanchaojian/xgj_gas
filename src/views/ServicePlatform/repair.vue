...
-->
<template>
    <div id="repair" name="repair" style="width:100%; height:100%; background-color:white;">
        <!--头部区域-->
			...
			<div v-loading="loadingData" :style="dataBox" class="container-fluid">
				...
				<div  v-for="(repair,i) in search(keywords)" :id="repair.Id" :index="i" :key="repair.Id" class="col-xs-12 container-fluid splitLine">
					<div id="item" class="col-xs-12" style="padding:15px 0px;">
						<div @click.stop="toDetails(repair)" class="col-xs-9"><p style="font-size:14px; font-weight:400;">{{repair.Name}}</p></div>
						<div class="col-xs-2"><span style="font-size:12px;">签到</span><span :class="{'sbox':true,'finished':repair.SignState__c=='已签到' ? true:false,'nFinished':repair.SignState__c=='已签到' ? false:true}"></span></div>
					...
					<transition>
						<div id="details" class="col-xs-12" style="padding-bottom:10px; display:none; padding-left:15px;">
							...
							<div class="ds ln1">编号：{{repair.Equipment__r.Name}}</div>
							<div class="ds ln2">名称：{{repair.Equipment__r.DeviceName__c}}</div>
							<div class="ds ln2">报修时间：{{repair.SubmitTime__c | dataformat}}</div>
							<div class="ds ln2">约定时间：{{repair.AppointTime__c | dataformat}}</div>
							<div  class="col-xs-12 ln2" style=" padding-top:5px;">
								<div class="col-xs-10 ds">地址：{{repair.Equipment__r.DeviceAddress__c}}</div>
								<a :href="toAmap(repair)" class="col-xs-2 ds" align="right">导航<a class="glyphicon glyphicon-send ds"></a></a>
							</div>
				...
				<!--=====================================================-->
				<div v-if="repairListAll.length == 0"  style="text-align:center;">
					<div>你目前还没有维修任务呢。</div>
				</div>	
			</div>
...
export default {
    ...
    methods:{
		...
		search:function(keywords){  //根据关键字对数据进行搜索
			var newList = [] ;
			this.repairListAll.forEach(item => {
				if(item.Name.indexOf(keywords) != -1 || item.Equipment__r.Name.indexOf(keywords) != -1){ //搜索到了
					newList.push(item);
				}
			});
			return newList ; 
		...
		//导航地址处理
		toAmap(repair){
			var address = repair.Equipment__r.DeviceAddress__c ; 
			if('LongitudeAndLatitude__c' in repair.Equipment__r){
				var lng = repair.Equipment__r.LongitudeAndLatitude__c.split(/,|，/)[0] ;
				var lat = repair.Equipment__r.LongitudeAndLatitude__c.split(/,|，/)[1] ;
		...
		toDetails(repair){
			...
			this.$store.commit('setRepairDetails',params) ;
			this.$router.push({
				name:"repairDetails",
			}) ;
		},
