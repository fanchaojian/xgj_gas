...
-->
<template>
    <div id="dispatchBill" name="dispatchBill" style="width:100%; height:100%; background-color:white;">
       <!--头部区域-->
		...
		<mt-loadmore 
			...
			<div v-loading="loadingData" :style="dataBox" class="container-fluid">
				<!--任务列表，遍历体-->
				<div  v-for="(dispatchBill,i) in search(keywords)" :id="dispatchBill.Id" :index="i" :key="dispatchBill.Id" class="col-xs-12 splitLine" >
					<div id="item" class="col-xs-12" style="line-height:50px; ">
						<div @click.stop="toDetails(dispatchBill)" class="col-xs-8">{{dispatchBill.Name}}</div>
						<div class="col-xs-3" align="center"><span style="font-size:12px;">签到</span><span :class="{'sbox':true,'finished':dispatchBill.SignState__c=='已签到' ? true:false,'nFinished':dispatchBill.SignState__c=='已签到' ? false:true}"></span></div>
				...
				<div class="col-xs-12" v-if="dispatchBillListAll.length == 0"  style="text-align:center;">
					<div>你目前还没有送货任务呢。</div>
				</div>
			</div>
		</mt-loadmore>
...
export default {
    ...
    methods:{
		...
		search:function(keywords){  //根据关键字对数据进行搜索
			var newList = [] ;
			this.dispatchBillListAll.forEach(item => {
				if(item.Name.indexOf(keywords) != -1){ //搜索到了
					newList.push(item);
				}
			});
			return newList ; 
		...
		loadTop() {
			...
			this.getDispatchBill() ; 
			
			this.$refs.loadmore.onTopLoaded();
		},
		toDetails(dispatchBill){
			//将数据放入store
			var params = {
					'dispatchBill': dispatchBill ,
					'keywords':this.keywords,
				}
			this.$store.commit('setDispatchBillDetails',params) ;
			this.$router.push({
		...
		getDispatchBill(){
			...
			_this.dispatchBillListAll = [] ;    //清空数据集

			_this.$request_SF({
				method:"get",
				url:"/PlatformDispatchBillTaskRest/getTask",
				params:{
				...
				if(res.data.length>0 || res.statusText == "OK"){
					_this.dispatchBillListAll = res.data ; 
					//加载动画停止
