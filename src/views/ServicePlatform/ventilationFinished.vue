				...
				<div  v-for="(ventilation,i) in search(keywords)" :id="ventilation.Id" :index="i" :key="ventilation.Id" class="col-xs-12 container-fluid splitLine">
					...
					<transition>
						<div id="details" class="col-xs-12" style="padding-bottom:10px; display:none; padding-left:15px;">
							...
							<div class="ds ln1">设备编号：{{ventilation.Device__r.Name}}</div>
							<div class="ds ln2">设备名称：{{ventilation.Device__r.DeviceName__c}}</div>
                            <div class="ds ln2">签到/退人：{{ventilation.Signer__r.Name}}</div>
...
export default {
    ...
    methods:{
		...
		getVentilation(){
			...
			_this.$request_SF({
				method:"get",
				url:"/QueryVentilationByEquidRest/getTask",
				params:{
